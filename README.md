...
- Implement a search routine for the drone to navigate a predefined area of interest. (DONE)
- Allow the drone to switch between the search and inspection routines dynamically. (DONE)
- Integrate adaptive path planning for the drone based on the detection.

## Improvements 
1. calculate yaw angle at current position to publish search waypoints at the correct YAW Angle (DONE)
2. Optimize the search and inspection time. (DONE)
3. Fix the case when it's hovering and cannot see the apriltag because it keeps yawing in place (DONE)

## BIG IMPROVEMENTS 
1. BIG IMPROVEMENT IS TO GET THE APRILTAG OUT OF THE INSPECTION ROUTINE !!!! THINK OF SAVING THE APRIL TAG POSE IN THE INERTIAL FRAME ONCE IT"S BEEN SEEN AND THEN CALCULATE THE PUBLISHED OFFSET BASED ON THE DESIRED OFFSET LOADED FROM CONFIG FILE AND APRIL TAG POSE IN THE INERTIAL FRAME (DONNNNNNEE)
